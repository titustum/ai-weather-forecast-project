...
class Trainer():
	...
	def __init__(self, condition='temperature', city='Toronto', epochs=1, datasamples=0):
	...
	def train(self):
		...
		# train the model
		self.model=model.fit(x_train, y_train, batch_size=1, epochs=self.epochs)
		...
		self.predictions=scaler.inverse_transform(predictions)
		...
		model.save(f"../models/{self.city}_{self.condition}_{self.datasamples}_model_{myrmse}.h5")
	...
	def set_trained_model(self, model_name):
		self.model=load_model(f"../models/{model_name}.h5")	
		self.predictions=self.model.predict(self.get_x_test())

	def get_predictions(self):
		predictions=self.scaler.inverse_transform(self.predictions)
		self.valid["Predictions"]=predictions
		return predictions